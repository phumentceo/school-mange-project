            ...
            <form id="teacherForm" class="row g-3" method="POST" action="{{ route('admin.teacher.update', $teacher->id) }}">
                ...
                <div class="col-md-12">
                    ...
                    <select name="gender" class="form-control shadow-none @error('gender') is-invalid @enderror">
                        ...
                        <option value="1" {{ old('gender', $teacher->gender) == '1' ? 'selected' : '' }}>ភេទប្រុស
                        </option>
                        <option value="2" {{ old('gender', $teacher->gender) == '2' ? 'selected' : '' }}>ភេទស្រី
                        </option>
                    </select>
                    @error('gender')
                        <div class="invalid-feedback">{{ $message }}</div>
                ...
                <div class="col-md-6">
                    ...
                    <select name="marital_status"
                        ...
                        <option value="1"
                            {{ old('marital_status', $teacher->marital_status) == '1' ? 'selected' : '' }}>មានគ្រួសារ
                        </option>
                        <option value="2"
                            {{ old('marital_status', $teacher->marital_status) == '2' ? 'selected' : '' }}>នៅលីវ</option>
                    </select>
                    ...
                    <select name="subject_id[]" id="subjects" class="form-control shadow-none @error('subject_id') is-invalid @enderror" multiple="multiple">
                        ...
                        @foreach ($subjects as $subject)
                            <option value="{{ $subject->id }}"
                                {{ old('subject_id', $subject->teacher_id) == $teacher->id ? 'selected' : '' }}>
                ...
                <div class="col-6">
                    ...
                    <input type="text" class="form-control shadow-none @error('degree') is-invalid @enderror"
                        name="degree" value="{{ old('degree', $teacher->degree) }}" placeholder="បរិញ្ញាប័ត្រ">
                    @error('degree')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="col-6">
                    ...
                    <input type="text" class="form-control shadow-none @error('university') is-invalid @enderror"
                        name="university" value="{{ old('university', $teacher->university) }}">
                    @error('university')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
